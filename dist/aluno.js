...
var RenderSecVirtual = /** @class */ (function () {
    function RenderSecVirtual(content, aulas, dias) {
        var _this = this;
        this.deleteBody = function () {
            _this.body.removeChild(_this.childLink);
        };
        this.setClasses = function () {
            _this.body.className = "bg-sky-100";
            _this.nav.className = "relative w-full flex flex-wrap items-center justify-between bg-blue-600 py-4 text-yellow-50 hover:text-gray-700 focus:text-gray-700 shadow-lg navbar navbar-expand-lg navbar-light";
            _this.footer.className = "container-fluid w-full flex flex-wrap items-center px-6 h-fit bg-blue-600 justify-left";
            _this.section.className = "block h-fit mt-10 mb-10 flow-root w-full flex justify-center mx-auto";
            _this.LeftDiv.className = "flex flex-row w-full justify-center";
            _this.dummyTimer.setAttribute("id", "divInformacaoTimeout");
            _this.dummyTimer.className = "hidden";
        };
        this.setInnerHTML = function () {
            ...
            Object.values(_this.ClassInfo).map(function (el) {
            ...
            var classCount = 0;
            Object.values(BufferAulas).map(function (el) {
                if (el.length > 0) {
                    el.map(function (el, index) {
                        var inicio = {
                            hours: _this.aulas[classCount].attributes[5].value.split("INÍCIO:")[1].split("DURAÇÃO")[0].includes(",5") ? _this.aulas[classCount].attributes[5].value.split("INÍCIO:")[1].split("DURAÇÃO")[0].split(",")[0] : _this.aulas[classCount].attributes[5].value.split("INÍCIO:")[1].split("DURAÇÃO")[0].split("h")[0],
                            minutes: _this.aulas[classCount].attributes[5].value.split("INÍCIO:")[1].split("DURAÇÃO")[0].includes(",5") ? "30" : "00"
                        };
                        var duration = {
                            hours: _this.aulas[classCount].attributes[5].value.split("DURAÇÃO:")[1].split('LOTAÇÃO')[0].includes(",5") ? _this.aulas[classCount].attributes[5].value.split("DURAÇÃO:")[1].split('LOTAÇÃO')[0].split(",")[0] : _this.aulas[classCount].attributes[5].value.split("DURAÇÃO:")[1].split('LOTAÇÃO')[0].split("h")[0],
                            minutes: _this.aulas[classCount].attributes[5].value.split("DURAÇÃO:")[1].split('LOTAÇÃO')[0].includes(",5") ? "30" : "00"
                        };
                        var time = addTimes([inicio, duration]);
                        var i = 0;
                        switch (el[0]) {
                            case "Segunda-Feira":
                                i = 0;
        ...
        this.appendElements = function () {
            _this.header.appendChild(_this.nav);
            _this.body.appendChild(_this.header);
            _this.section.appendChild(_this.LeftDiv);
            _this.body.appendChild(_this.section);
            _this.body.appendChild(_this.dummyTimer);
        };
        this.setListeners = function () {
            var observer = new MutationObserver(function (mutations) {
                var _a;
                if ((_a = _this.dummyTimer.textContent) === null || _a === void 0 ? void 0 : _a.includes("1.59")) {
                    var button = document.getElementsByClassName("jqSessionAlerterButton reset")[0];
                    button.click();
                }
            });
            var element = document.body.querySelector("span#divInformacaoTimeout");
        ...
        this.dummyTimer = document.createElement("span");
        this.dadosContent = {};
...
var TableDadosParser = /** @class */ (function () {
    ...
    ;
    TableDadosParser.prototype.getSchedule = function (ScheD) {
        var _this = this;
        var schedule = ScheD.body.querySelectorAll('#template_main > table')[0];
        var lines = schedule.querySelectorAll('tbody > tr:nth-child(n+3):nth-child(-n+8) > td[class="horario_turma"]');
        this.lines = lines;
        var dias;
        (function (dias) {
            dias[dias["Segunda"] = 0] = "Segunda";
            dias[dias["Ter\u00E7a"] = 1] = "Ter\u00E7a";
            dias[dias["Quarta"] = 2] = "Quarta";
            dias[dias["Quinta"] = 3] = "Quinta";
            dias[dias["Sexta"] = 4] = "Sexta";
        })(dias || (dias = {}));
        this.lines.forEach(function (el, index) {
            var _a, _b, _c, _d, _e;
            var title = el.attributes[5];
            if (title.value.includes(dias[0])) {
                _this.classInfo["Aula" + index] = ["Segunda-Feira", (_a = el.textContent) === null || _a === void 0 ? void 0 : _a.split("(")[0].split("\n")[1]];
            }
                ...
                _this.classInfo["Aula" + index] = ["Sexta-Feira", (_e = el.textContent) === null || _e === void 0 ? void 0 : _e.split("(")[0].split("\n")[1]];
            }
            ;
        });
    };
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        text = [];
                        return [4 /*yield*/, Promise.all(links.map(function (el) { return fetch(el, { headers: myHeaders })
                                ...
                                .catch(function (e) { return console.warn(e.message); })];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
